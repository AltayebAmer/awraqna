...
### Search Console — **مُتحقَّق ومُقدَّم** ✓

الخاصية: **URL prefix** على `https://awraqna.com/`، بطريقة **ملف HTML**:
`google8b4519a585720f3d.html` في جذر المستودع.

> **لا تحذف هذا الملف أبداً** — حذفه يُلغي التحقّق ويوقف التقارير.
> و`build.py` و`preflight.py` يتخطّيانه (ليس صفحة موقع).

**لماذا URL prefix لا Domain property:** تحقّق Domain يحتاج سجلّ DNS
يضيفه Google عبر نافذة موافقة Cloudflare تُفتح كـ **popup** — والـ popup
خارج مجموعة تبويبات الوكيل، فالخطوة غير قابلة للأتمتة. طريقة الملف
تعطي نفس النتيجة لهذا النطاق بلا منح أي صلاحية على الحساب.

**فخّ Cloudflare Pages:** الاستضافة تحذف امتداد `.html` وتردّ `308` من
`/google….html` إلى `/google…`. جوجل يتبع التوجيه فيتحقّق بنجاح — لكن
لا تفزع إن رأيت `308` في `curl`.

**ما تمّ فعلاً:**

| العنصر | الحالة |
|---|---|
| `sitemap.xml` | **Success · ٤١ صفحة مكتشَفة** |
| `https://awraqna.com/` | كانت مفهرسة · أُعيد طلب الفهرسة ✓ |
| `https://awraqna.com/worksheets/` | «أُضيفت إلى قائمة الزحف ذات الأولوية» ✓ |
| `https://awraqna.com/articles/` | «أُضيفت إلى قائمة الزحف ذات الأولوية» ✓ |

**ملاحظة على الحصّة:** طلب الفهرسة محدود يومياً، وقد يردّ جوجل بخطأ
عابر «واجهنا مشكلة في إرسال طلبك». أعد المحاولة بعد دقيقة — وقع هذا مرة
ونجح في الثانية. **تحقّق دائماً بإعادة فحص الرابط**: ظهور «أطلب مجدداً»
مكان «طلب فهرسة» هو الدليل الوحيد الموثوق على التسجيل.

**الخطوة التالية بعد أسبوع:** راجع Pages ← لمعرفة كم صفحة فُهرست فعلاً
من الـ٤١، وعالج ما يظهر تحت «لم تتم الفهرسة».
